...
/**
 * @brief Executes the TES_C07 function.
 *
 * This function is responsible for executing the TES_C07 function, which performs a series of operations on a set of exercise folders.
 ...
 * If an exercise folder is found, it prints a success message and calls the z_test_C07_ex_folder function to perform further operations.
 ...
 *
 * @note The get_valid_days_folders, check_if_ex_folder_exist, z_test_C07_ex_folder, and z_display_Summary_Rusults functions are assumed to be defined elsewhere.
 *
 * @param None
 * @return None
 */

void zi_TES_C07() /* day : C07 */
{
    char *exrcices[] = {"ex00", "ex01", "ex02", "ex03", "ex04", "ex05"};
    char **curunt_ex_folders = get_valid_days_folders(exrcices, MAX_EX_C07); //@ NEED freeing
    ...
    {
        if (check_if_ex_folder_exist(exrcices, curunt_ex_folders[i], MAX_EX_C07) == 1)
        {
            printf("\t\t\t%s---------------------------%s\n", purple, reset);
            printf("\t\t\t%s| [ ✔️ ] Folder %s Found |%s\n", purple, curunt_ex_folders[i], reset);
            printf("%s--------------------------------------------------------------------------%s\n", purple, reset);
            z_test_C07_ex_folder(curunt_ex_folders[i], status_exr, &pos);
        }
        else
        {
