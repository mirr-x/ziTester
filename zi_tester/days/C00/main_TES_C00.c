 ...
 * 
 * This function is responsible for executing the TES_C01 function, which performs a series of operations on a set of exercise folders.
 ...
 * If an exercise folder is found, it prints a success message and calls the z_test_C01_ex_folder function to perform further operations.
 ...
 * 
 * @note The get_valid_days_folders, check_if_ex_folder_exist, z_test_C01_ex_folder, and z_display_Summary_Rusults functions are assumed to be defined elsewhere.
 * 
 * @param None
 * @return None
 */
...
void zi_TES_C00() /* day : C00 */
{
    ...
    while (curunt_ex_folders[i] != NULL)
    {
        if (check_if_ex_folder_exist(exrcices, curunt_ex_folders[i], MAX_EX_C00) == 1)
        {
            ...
            printf("%s--------------------------------------------------------------------------%s\n", purple, reset);
            z_test_C00_ex_folder(curunt_ex_folders[i], status_exr, &pos);
        }
        else
        {
